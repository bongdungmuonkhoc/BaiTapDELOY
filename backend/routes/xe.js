...
router.get('/xe', (req, res) => {
  ...
  if (search) {
    ...
    const sql = `
      ...
      WHERE LOWER(Ten_Xe) LIKE ? OR LOWER(Hang_Xe) LIKE ? OR LOWER(Bien_So) LIKE ? OR LOWER(Loai) LIKE ?
    `;
...
router.get('/xe/:id', (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    res.json(results[0]); // ✅ trả về object
  });
});
...
router.get("/xe", (req, res) => {
  ...
  if (loai === "5chỗ") {
    sql = `
      SELECT xe.Ma_Xe, xe.Ten_Xe, xe_5cho.So_Cho_Ngoi, xe_5cho.Mo_Ta, xe_5cho.Hinh_Anh
      FROM xe
      JOIN xe_5cho ON xe.Ma_Xe = xe_5cho.Ma_Xe
    `;
  } else if (loai === "7chỗ") {
    sql = `
      SELECT xe.Ma_Xe, xe.Ten_Xe, xe_7cho.So_Cho_Ngoi, xe_7cho.Mo_Ta, xe_7cho.Hinh_Anh
      FROM xe
      JOIN xe_7cho ON xe.Ma_Xe = xe_7cho.Ma_Xe
    `;
  } else if (loai === "bán tải") {
    sql = `
      SELECT xe.Ma_Xe, xe.Ten_Xe, xe_bantai.Tai_Trong, xe_bantai.Mo_Ta, xe_bantai.Hinh_Anh
      FROM xe
